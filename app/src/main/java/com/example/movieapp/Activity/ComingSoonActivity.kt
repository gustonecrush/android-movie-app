...
class ComingSoonActivity : AppCompatActivity(), OnMovieClickListener {
    ...
    private fun getMovies() {
        ...
        retro.getMoviesByCategory("upcoming", BuildConfig.MOVIEDB_API_KEY).enqueue(object:
            Callback<MovieResponse> {
            override fun onResponse(call: Call<MovieResponse>, response: Response<MovieResponse>) {
                response.body()?.results?.let{ list.addAll(it) }
                rv_list_movie.apply {
                    layoutManager = LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false)
                    adapter = MovieAdapter(list, "upcoming", false, this@ComingSoonActivity)
                }
